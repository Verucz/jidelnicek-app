...
].map((r) => ({ ...r, verified: true, custom: false }));
  ...
  const multiDayPlan = useMemo(() => {
    ...
    for (let d = 0; d < form.days; d++) {
      const daySlots = slots.map((slot) => {
        ...
        const options = pickThree(fullPool, slot.cat, target, d + manualOffset);
        return { ...slot, key, target, options };
      });
      days.push({ dayIndex: d, slots: daySlots });
    }
    return days;
  }, [form.days, form.mealsPerDay, results.tdee, offsets, slots, fullPool]);
